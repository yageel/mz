...
(function () {
    ...
    loadAllImage(function () {
        ...
        if (tools.session("redMoney-dadi")) {
            var sessionData = JSON.parse(tools.session("redMoney-dadi"));
    ...
    function startGame() {
        ...
        if (redrain_status == 1) {
            ...
            tools.alert("红包雨活动还没有开始，请小主耐心等候哦~",function(){
            	 location.href =tools.url("index", "index", { item: adid });
            });

        }else if(redrain_status ==2){
        	gameState = 3;
      
            tools.alert("红包雨活动已经结束，请点击确定退出页面哦~",function(){
           	 	 location.href =tools.url("index", "index", { item: adid });
            });
        } else {
            gameState = 0;
    ...
    function timeOver() {
	     ...
	     tools.session("redMoney-dadi", JSON.stringify(sessionData));
    };
    ...
    // 添加游戏提示文字
    function addPrompt() {
        var prompt = keySprites.prompt = new PIXI.Text("点击屏幕红包和金币团，开抢把！");
