...
(function () {
    var resBasePath = "/Public/images/redRain/",
        ...
        totalSecond = 15, gameState, goldScore = redScore = 0,
    ...
    loadAllImage(function () {
        ...
        if (tools.session("redMoney")) {
            var sessionData = JSON.parse(tools.session("redMoney"));
    ...
    // 添加游戏对象（现金或M币）
    function addGameObj() {
        var obj = new gameObject(Math.random() > .7 ? "red" : "gold");
    ...
    function startGame() {
        ...
        if (userSubscribe ==0 && tools.queryPars("from") === "1") {
           ...
           tools.ajax(tools.url("active", "get_user_subscribe"), function (ret) {
                tools.closeLoading();
    ...
    // 游戏计时结束
    function timeOver() {
    	tools.ajax(tools.url("active", "redrain_gameresult"), {
            uuid: actUId,
            id: actSubId
    	}, function (ret) {
    	    if (ret.state == 10) {
    	    	if(actSource ==1){
    	    		//ipm
    	    		 var sessionData = { msg: "恭喜您一共获得" + ret.data.red_money + "元红包!快去提现吧~", gotoLink: tools.url("active", "lottery", {id: actUId }) };
    	    	}else{
    	    		//大地
    	    		var adid=$("#redrain_adid").val();
    	    		if(tools.queryPars("from") === "1"){
    	    			var sessionData = { msg: "恭喜您一共获得" + ret.data.red_money + "元红包!快去提现吧~~", gotoLink: tools.url("index", "index", { item: adid }) };
    	    		}else{
    	    			//var sessionData = { msg: "恭喜您一共获得" + ret.data.red_money + "元红包!快去提现吧~~~", gotoLink: tools.url("active", "redrainlottery", { id: actUId,subid: actSubId}) };
    	    			var sessionData = { msg: "恭喜您一共获得" + ret.data.red_money + "元红包!快去提现吧~~", gotoLink: tools.url("index", "index", { item: adid }) };
    	    		}
    	        ...
    	        tools.session("redMoney", JSON.stringify(sessionData));

                //游戏结果
    	        tools.alert(sessionData.msg, function () {
                    //跳到结算页
    	            location.href = tools.url("active", "redrain_settlement", { uuid: actUId, id: actSubId });
                });
    	    } else {
    	        tools.session("redMoney", JSON.stringify({ msg: ret.msg, gotoLink: "" }));
    ...
    // 添加游戏提示文字
    function addPrompt() {
        var prompt = keySprites.prompt = new PIXI.Text("点击屏幕红包和金币团，开抢吧!");
        ...
        return function (type) {
            ...
            icon.touchstart = function (e) {
                ...
                if (gameState == 1) {
                    ...
                    tools.ajax(tools.url("active", "redrain_winred"), {
                        raintype: pritype,
                        uuid: actUId,
                        id: actSubId
                    }, function (ret) {
    ...
    function addScore() {
        ...
        redScoreCircle.visible = goldScoreCircle.visible = false;

        goldScoreText.position.x = 215;
        goldScoreText.style.fill = redScoreText.style.fill = scoreLabel.style.fill = "#ffffff";
        goldScoreText.style.font = redScoreText.style.font = scoreLabel.style.font = "bold 40px Arial";
        goldScoreText.anchor.y = goldScoreIcon.anchor.y = redScoreText.anchor.y = redScoreIcon.anchor.y = scoreLabel.anchor.y = .5;
        redScoreCircle.position.y = goldScoreCircle.position.y = goldScoreText.position.y = goldScoreIcon.position.y = redScoreText.position.y = redScoreIcon.position.y = scoreLabel.position.y = score.height / 2 + 8;
        ...
        goldScoreIcon.position.x = 155;
        
        redScoreText.position.x = 89;
        ...
        redScoreIcon.position.x = -5;

        scoreLabel.position.x = -265;
