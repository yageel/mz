...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    /**
     * 获取JS 微信卡券API授权TICKET 获取api_ticket
     * @param string $appid 用于多个appid时使用,可空
     * @param string $api_ticket 手动指定jsapi_ticket，非必要情况不建议用
     */
    public function getWxTicket($appid = '', $api_ticket = '') {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        if (!$appid)
            $appid = $this->appid;
        ...
        if ($result) {
            ...
            $expire = $json['expires_in'] ? intval($json['expires_in']) - 100 : 3600;
            $token['access_token'] = $json['ticket'];
            $token['expires_in'] = time() + $expire;
     ...
     * @return array|bool 返回签名字串
     */
    public function getJsSignCard($url, $card_id = '', $card_type = '', $timestamp = 0, $noncestr = '', $appid = '') {
        if (!$this->api_ticket && !$this->getWxTicket($appid) || !$url)
            return false;
        if (!$timestamp)
            $timestamp = time();
        if (!$noncestr)
        ...
        if (empty($url))
            return false;
        //$arrdata = array("timestamp" => $timestamp, "noncestr" => $noncestr,"api_ticket" => $this->api_ticket,'card_id'=>$card_id,'card_type'=>$card_type,'app_id'=>$this->appid);
        //print_r($arrdata);
        //$sign = $this->getCardSignature($arrdata);
        // code=jonyqin_1434008071，timestamp=1404896688，card_id=pjZ8Yt1XGILfi-FUsewpnnolGgZk， api_ticket=ojZ8YtyVyr30HheH3CM73y7h4jJE ，nonce_str=jonyqi
        $sign = sha1("$timestamp{$this->api_ticket}$card_id");
        if (!$sign)
            return false;
        $signPackage = array(
            "appid" => $this->appid,
        ...
        if ($result) {
            if (is_string($result)) {
                ...
                if (isset($json['errcode'])) {
                    $this->errCode = $json['errcode'];
                    $this->errMsg = $json['errmsg'];
                    @file_put_contents('/data/log/xqb.log', strftime("%Y-%m-%d %H:%M:%S") . 'getMedia res:' .$result. "\n", FILE_APPEND);
                    return false;
                }
            }
            return $result;
    ...
    public function __destruct(){
        if($this->errCode > 0){
            if($this->errCode == 40001){
                $this->removeCache('wechat_access_token'.$this->appid);
            }
            $url = 'http://'.$_SERVER['SERVER_NAME'].$_SERVER["REQUEST_URI"];
            File::write_file(APP_PATH .'log/error6.log',date("Y-m-d H:i:s")."url={$url}&errCode=".$this->errCode ."appid=".$this->appid."&errMsg=".$this->errMsg."\r\n",'a+');
        }
